...
export default function RepReach() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    setApolloLeads([]);
    setLeads([]);
    setSearchQuery(retailerName || "search results");
    setScreen("results");
    setSearching(false);
    // Fetch Apollo results and populate leads directly
    try {
      const res = await fetch("/api/search", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      const apolloResults = data.leads || [];
      setLeads(apolloResults);
      setApolloLeads(apolloResults);
      if (apolloResults.length > 0) setActiveLead(apolloResults[0]);
    } catch(e) {
...
<div className="feature"><div className="feature-icon">🎯</div><h3>Verified Decision-Makers Only</h3><p>Every result is the actual buyer, category manager, or merchant who controls your product's shelf space — with their direct email and phone number. No gatekeepers, no switchboards, no dead ends.</p></div>
      ...
      {screen === "results" && (
        <div className="page">
          ...
          {leads.length === 0 ? (
            <div>
              {apolloLoading || searching ? (
                <div style={{textAlign:"center",padding:"60px 0"}}>
                  <span className="spin spin-d" style={{width:32,height:32,display:"inline-block",marginBottom:16}}/>
                  <div style={{fontSize:16,fontWeight:700,color:"var(--text)"}}>Searching Apollo database...</div>
                  <div style={{fontSize:13,color:"var(--muted)",marginTop:8}}>Finding buyers at {searchQuery}</div>
                </div>
              ) : (
