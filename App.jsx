...
export default function App() {
  ...
  const runSearch = useCallback(async (company, titles) => {
    ...
    try {
      ...
      setLeads(r); setTotalAvailable(total); setHasSearched(true); setActiveLead(null);
      setDepartments({});
  ...
  // Fetch departments for visible leads using web search
  const fetchDepartments = useCallback(async (leadList) => {
    if (!leadList.length) return;
    setLoadingDepts(true);
    // Only fetch for contacts we don't already have
    const needed = leadList
      .filter(l => !departments[l.id])
  ...
  const handleCompanyInput = (val) => {
    ...
    searchTimer.current = setTimeout(() => runSearch(val, selectedTitles.length ? selectedTitles.join(" ") : null), 500);
  };
          ...
          <div className="topbar">
            <div className="ts-wrap">
              ...
              <input autoFocus value={companyInput}
                ...
                placeholder="Search any retailer to find buyers instantly..." />
            </div>
            ...
            <div className="main">
              ...
              {view === "people" ? (<>
                <div className="toolbar">
                  {searching ? (
                    <><span className="spin"/><span style={{color:"#334155",marginLeft:8,fontWeight:600}}>Finding buyers at <span style={{color:"#00c9a7"}}>{companyInput}</span>...</span></>
                  ) : hasSearched ? (
                    <>
