export default async function handler(req, res) {
  ...
  const post = (url, body) =>
    fetch(url, { method:"POST", headers:HEADERS, body:JSON.stringify({...body, api_key:KEY}) });
  ...
  try {
    ...
    if (domain) {
      const r = await fetch(`https://api.apollo.io/v1/organizations/enrich?domain=${domain}&api_key=${KEY}`);
